...
type UndergroundSystem struct {
	customers map[int]customerStatistic
	stations  map[string]stationStatistic
}
...
func (this *UndergroundSystem) CheckIn(id int, stationName string, t int) {
	this.customers[id] = customerStatistic{
...
func (this *UndergroundSystem) CheckOut(id int, stationName string, t int) {
	...
	station := customer.startStation + "_" + stationName
	wholeTime := t - customer.startTime
	if _, ok := this.stations[station]; ok {
		stationStat := this.stations[station]
		stationStat.time += wholeTime
		stationStat.count++
		this.stations[station] = stationStat
	} else {
		this.stations[station] = stationStatistic{
			time:  wholeTime,
			count: 1,
		}
	}

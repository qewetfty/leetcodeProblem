...
func findDuplicate(paths []string) [][]string {
	...
	for _, files := range paths {
		...
		fileNum := len(split)
		for i := 1; i < fileNum; i++ {
			fileAndContent := split[i]
			bracketIndex := strings.Index(fileAndContent, "(")
			fileName, content := fileAndContent[:bracketIndex], fileAndContent[bracketIndex+1:len(fileAndContent)-1]
			contentMap[content] = append(contentMap[content], path+"/"+fileName)
		}
	}

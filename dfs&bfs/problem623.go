...
func addOneRow(root *data.TreeNode, v int, d int) *data.TreeNode {
	if d == 1 {
		newRoot := &data.TreeNode{Val: v, Left: root}
		return newRoot
	}
	...
	for len(queue) > 0 {
		...
		if depth == d {
			for _, node := range queue {
				...
				node.Left = &data.TreeNode{Val: v, Left: originLeft}

				originRight := node.Right
				node.Right = &data.TreeNode{Val: v, Right: originRight}

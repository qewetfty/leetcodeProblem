...
func openLock(deadends []string, target string) int {
	if strings.EqualFold("0000", target) {
		return 0
	}
	deadlockMap := make(map[string]bool)
	for _, deadend := range deadends {
		deadlockMap[deadend] = true
	}
	queue := make([]string, 0)
	...
	for len(queue) > 0 {
		...
		for i := 0; i < l; i++ {
			curPassword := queue[i]
			// deadend，继续处理下一个
			if _, ok := deadlockMap[curPassword]; ok {
				continue
			}
			// 找到了就返回结果
			if strings.EqualFold(curPassword, target) {
				return step
			}
			// 没找到就遍历下一次的所有结果
			for j := 0; j < 4; j++ {
				curChar := curPassword[j]
				// 先加一
				addChar := curChar + 1
				if addChar > '9' {
					addChar = '0'
				}
				newAddPass := curPassword[:j] + string(addChar) + curPassword[j+1:]
				if !visited[newAddPass] {
					queue = append(queue, newAddPass)
					visited[newAddPass] = true
				}
				// 再减一
				minChar := curChar - 1
				if minChar < '0' {
				...
				newMinPass := curPassword[:j] + string(minChar) + curPassword[j+1:]
				if !visited[newMinPass] {
					queue = append(queue, newMinPass)
					visited[newMinPass] = true
				}
			}
		}
		queue = queue[l:]

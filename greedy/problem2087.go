...
//is the bottom-right cell. You are given an integer array startPos where startPos
...
//incurs some cost. You are further given two 0-indexed integer arrays: rowCosts of
...
//
// Related Topics Array Greedy Matrix 👍 132 👎 30

//leetcode submit region begin(Prohibit modification and deletion)
func minCost(startPos []int, homePos []int, rowCosts []int, colCosts []int) int {
	r1, c1, r2, c2 := startPos[0], startPos[1], homePos[0], homePos[1]
	cost := 0
	if r1 <= r2 {
		for i := r1 + 1; i < r2+1; i++ {
			cost += rowCosts[i]

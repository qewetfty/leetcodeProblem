...
func slowestKey(releaseTimes []int, keysPressed string) byte {
	l := len(releaseTimes)
	timeList := make([]int, 26)
	timeList[keysPressed[0]-'a'] = releaseTimes[0]
	for i := 1; i < l; i++ {
		timeList[keysPressed[i]-'a'] = utils.Max(releaseTimes[i]-releaseTimes[i-1], timeList[keysPressed[i]-'a'])
	}
	var result byte
	result = 'a'
	max := timeList[0]
	for i := 1; i < 26; i++ {
		if timeList[i] >= max {
			max, result = timeList[i], byte('a'+i)
		}
	}
	return result
}

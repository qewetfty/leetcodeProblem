...
func findTheWinner(n int, k int) int {
	return helper1823(n, k) + 1
}

func helper1823(n, k int) int {
	if n == 1 {
		return 1
	}
	return (helper1823(n-1, k) + k) % n
}
...
func findTheWinner2(n int, k int) int {
	...
	for len(array) > 1 {
		...
		for curNum >= len(array) {
			curNum = curNum - len(array)
		}
		array = append(array[:curNum], array[curNum+1:]...)
	}
	return array[0]
}

func testProblem1823() {

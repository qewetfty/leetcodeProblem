...
//
//
// Related Topics Array String Union Find Graph 👍 2443 👎 36

//leetcode submit region begin(Prohibit modification and deletion)
func equationsPossible(equations []string) bool {
	unionFind := data.NewUnionFind(26)
	for _, equation := range equations {
		e := equation[1]
		if e == '!' {
			continue
		}
		l1, l2 := equation[0], equation[3]
		unionFind.Union(int(l1-'a'), int(l2-'a'))
	}
	for _, equation := range equations {
		e := equation[1]
		if e == '=' {
		...
		l1, l2 := equation[0], equation[3]
		if unionFind.IsConnected(int(l1-'a'), int(l2-'a')) {
			return false
		}
	}
	return true

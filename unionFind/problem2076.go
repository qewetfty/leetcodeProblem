...
func friendRequests(n int, restrictions [][]int, requests [][]int) []bool {
	unionFind := data.NewUnionFind(n)
	result := make([]bool, 0)
	for _, request := range requests {
		x, y := unionFind.Parent(request[0]), unionFind.Parent(request[1])
		if x != y {
			check := true
			for _, restriction := range restrictions {
				u, v := unionFind.Parent(restriction[0]), unionFind.Parent(restriction[1])
				if (x == u && y == v) || (x == v && y == u) {
					check = false
					break
				}
